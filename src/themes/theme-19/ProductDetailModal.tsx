              ...
              <div className="grid grid-cols-2 gap-2">
                {product.portions.map((portion) => (
                  <button key={portion.id} onClick={() => { setSelectedPortion(portion); setSelectedTags({}); }}
                    className={cn('px-3 py-2 rounded-xl text-[12px] font-medium transition-all border-2 min-w-0 flex flex-col items-center justify-center leading-tight text-center',
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {isUnfulfilled && (
                    <span className={cn("px-2 py-0.5 text-xs rounded-full transition-all",
                      isShaking ? "bg-primary text-primary-foreground animate-pulse" : "bg-primary/10 text-foreground")}>{requirementLabel}</span>
                  )}
                </div>
