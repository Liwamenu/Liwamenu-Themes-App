...
export function Footer() {
  ...
  const socialLinks = [
    ...
    { url: SocialLinks.tiktokUrl, icon: TikTok, label: "TikTok", color: "hover:bg-zinc-900" },
    ...
    <footer className="bg-card border-t border-border mt-10">
      <div className="container px-4 py-12">
        {socialLinks.length > 0 && (
          <div className="mb-10">
            <h3 className="text-center mb-6 text-lg font-bold tracking-tight text-[hsl(var(--brand-orange))]">{t("footer.followUs")}</h3>
        ...
        <div className="max-w-sm mx-auto">
          <h3 className="text-center mb-6 text-lg font-bold tracking-tight text-[hsl(var(--brand-orange))]">{t("footer.workingHours")}</h3>
          ...
          <div className="flex justify-center gap-3 mt-6 flex-wrap">
            {restaurant.reservationSettings?.isActive && (
              <Button onClick={() => setIsReservationOpen(true)} variant="outline" className="flex items-center gap-2 rounded-full border-primary/30 dark:!text-white hover:!bg-primary hover:!text-primary-foreground transition-all">
                <CalendarDays className="w-4 h-4" />
                <span>{t("reservation.button")}</span>
              </Button>
            )}
            <Button onClick={() => setIsSurveyOpen(true)} variant="outline" className="flex items-center gap-2 rounded-full border-special/30 dark:!text-white hover:!bg-special hover:!text-special-foreground transition-all">
