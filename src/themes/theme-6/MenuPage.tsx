        ...
        <div className="max-w-[600px] mx-auto px-4 py-2">
          <div className="flex gap-3 items-center">
            <button
              ...
              className={`shrink-0 h-9 px-3 rounded-full shadow-md flex items-center gap-2 text-sm font-medium transition-all ${
                waiterCooldown > 0
          ...
          <section className="space-y-3">
            {[...restaurant.externalPages]
              ...
              .map((page) => (
                <CategoryBanner
                  key={page.id}
                  name={page.buttonName}
                  isOpen={false}
                  onToggle={() => setSelectedExternalPage(page)}
                />
              ))}
          </section>
        )}
