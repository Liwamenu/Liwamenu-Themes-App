            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                ...
                <div className="flex items-center gap-2">
                  {items.length > 0 && (
                    <button onClick={handleClearCart} className="text-sm text-destructive hover:text-destructive/80 dark:text-white dark:hover:text-white/80 bg-destructive/10 hover:bg-destructive/20 px-3 py-1.5 rounded-full transition-colors">
                      {t('cart.clearCart')}
                    </button>
                  )}
                ...
                {items.length === 0 ? (
                  ...
                  <AnimatePresence mode="popLayout">
                    {items.map((item) => {
                      ...
                      return (
                        <motion.div key={item.id} layout initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex gap-4 bg-secondary/30 rounded-[6px] p-3 border border-border/50">
              ...
              {items.length > 0 && (
                <div className="p-5 border-t border-border space-y-4 bg-card">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-2xl font-display font-bold text-primary whitespace-nowrap truncate min-w-0">{formatPrice(total)}</span>
                  </div>
