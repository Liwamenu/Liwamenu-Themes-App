...
export function MenuPage() {
  ...
  return (
    <div className="theme-13 min-h-screen bg-background">
      ...
      <div className="container px-[4px] py-6 pb-24">
        ...
        {activeCategory !== CAMPAIGN_CATEGORY_ID &&
          filteredCategories.map((category) => (
            <section key={category.id} ref={(el) => (categoryRefs.current[category.id] = el)} className="mb-8">
              <div className="mb-4 py-1.5 px-4 bg-primary text-primary-foreground text-center rounded-[5px] shadow-[0_4px_0_0_rgba(0,0,0,0.18),_0_10px_24px_-4px_rgba(0,0,0,0.45)]">
      ...
      {!isCartOpen &&
        ...
        !showReservation &&
        !showTableSelection && (
          <div className="fixed top-[148px] right-4 z-50">
            <button
              onClick={handleOpenCallWaiterFloating}
              disabled={waiterCooldown > 0}
              className={`h-8 px-2.5 rounded-full shadow-md flex items-center gap-1.5 text-xs font-medium transition-all ${
                waiterCooldown > 0
