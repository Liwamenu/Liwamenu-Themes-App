    ...
    <>
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-card">
                <div className="flex items-center gap-3">
                  ...
                  <span className="px-2 py-0.5 bg-primary text-primary-foreground text-sm rounded-full">{items.length}</span>
                </div>
                <div className="flex items-center gap-2">
                  {items.length > 0 && (
                    <button onClick={() => setShowClearConfirm(true)} className="text-sm text-secondary hover:text-secondary/80 bg-secondary/10 hover:bg-secondary/20 px-3 py-1.5 rounded-lg transition-colors font-medium">
                      {t('cart.clearCart')}
                    </button>
                  )}
                ...
                {items.length === 0 ? (
                  ...
                  <AnimatePresence mode="popLayout">
                    {items.map((item) => {
                      ...
                      else if (item.product.isCampaign && portion.campaignPrice != null) price = portion.campaignPrice;
                        ...
                        <motion.div key={item.id} layout initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex gap-4 bg-card rounded-2xl p-3 shadow-sm">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <button onClick={() => handleRemoveItem(item.id, item.product.name)} className="p-1.5 text-muted-foreground hover:text-destructive transition-colors">
      ...
      <AlertDialog open={!!itemToRemove} onOpenChange={() => setItemToRemove(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmRemoveItem} className="bg-destructive hover:bg-destructive/90">{t('common.remove')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={showClearConfirm} onOpenChange={setShowClearConfirm}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>{t('common.cancel')}</AlertDialogCancel>
            <AlertDialogAction onClick={confirmClearCart} className="bg-destructive hover:bg-destructive/90">{t('cart.clearCart')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
...
export function CartButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      data-cart-button
      className={`w-16 h-16 rounded-2xl bg-cart text-cart-foreground shadow-lg flex items-center justify-center ${shouldShake ? 'animate-shake' : ''}`}
      style={{ boxShadow: '0 4px 20px hsla(142, 58%, 26%, 0.4)' }}
    >
