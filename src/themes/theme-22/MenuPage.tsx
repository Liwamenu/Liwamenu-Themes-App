...
import { Search, X, Bell, Star } from "lucide-react";
...
export function MenuPage() {
  ...
  return (
    <div className="theme-22 min-h-screen bg-background">
          ...
          <div className="flex gap-2 items-center">
            ...
            <div className="relative flex-1 min-w-0">
              ...
              <Input
                ...
                className="h-11 pl-11 pr-11 rounded-full border-2 border-[hsl(var(--sushi-red))]/15 bg-secondary text-sm focus:border-[hsl(var(--sushi-red))] transition-all"
              />
              {searchQuery && (
                <button
      ...
      {/* Recommended Products — text-only chip strip (no photos in this theme) */}
      {!searchQuery && recommendedProducts.length > 0 && (
        <section className="container px-4 pt-4 pb-2">
          <div className="flex items-end justify-center mb-3">
            ...
            <h2 className="font-script text-3xl text-[hsl(var(--sushi-red))] leading-none mx-2">
              {t("menu.recommended")}
            </h2>
            <Star className="w-4 h-4 text-[hsl(var(--sushi-amber))] fill-[hsl(var(--sushi-amber))] mb-1" />
          </div>
          <div className="flex gap-2 overflow-x-auto hide-scrollbar scroll-fade-x pb-2 px-1">
            {recommendedProducts.slice(0, 10).map((product) => (
              <motion.button
                ...
                className="flex-shrink-0 px-4 py-2 rounded-full border border-[hsl(var(--sushi-red))]/25 bg-card hover:bg-[hsl(var(--sushi-red))] hover:text-white hover:border-transparent transition-colors font-script text-2xl leading-none text-[hsl(var(--sushi-red))]"
              >
                {product.name}
              </motion.button>
            ))}
      ...
      <div className="container px-4 py-6 pb-24">
        {!searchQuery && campaignProducts.length > 0 && activeCategory === CAMPAIGN_CATEGORY_ID && (
          <section ref={(el) => (categoryRefs.current[CAMPAIGN_CATEGORY_ID] = el)} className="mb-6">
        ...
        {activeCategory !== CAMPAIGN_CATEGORY_ID &&
          filteredCategories.map((category) => (
            <section key={category.id} ref={(el) => (categoryRefs.current[category.id] = el)} className="mb-6">
              <div className="cat-banner">
                {category.image && (
                  <div className="cat-banner-image" style={{ backgroundImage: `url(${category.image})` }} />
                )}
                <div className="cat-banner-inner">
                  <div className="menu-section-title m-0">
                    <h2>{category.name}</h2>
        ...
        {/* External Page Buttons — script style. One per
          ...
          <section className="mb-8 space-y-2">
            {[...restaurant.externalPages]
              ...
              .map((page) => (
                ...
                >
                  <h2 className="font-script text-4xl text-[hsl(var(--sushi-red))] hover:text-[hsl(var(--sushi-amber))] transition-colors">
                    {page.buttonName}
                  </h2>
                </button>
              ))}
