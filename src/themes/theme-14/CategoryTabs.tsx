...
/* Cycle through emojis matching Prem1 categories style */
      ...
      <div ref={scrollRef} className="flex overflow-x-auto hide-scrollbar gap-2 px-4 py-3">
        {campaignTab && campaignTab.count > 0 && (
          <motion.button
            ...
            whileTap={{ scale: 0.94 }}
            className={cn(
              'flex-shrink-0 flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200',
          ...
          >
            ...
            <span className="truncate max-w-[100px]">{campaignTab.name}</span>
          </motion.button>
        )}
        ...
        {categories.map((category, idx) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.94 }}
              className={cn(
                'flex-shrink-0 flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-medium transition-all duration-200',
                isActive
                  ? 'bg-primary text-primary-foreground shadow-glow'
                  : 'bg-secondary text-secondary-foreground hover:bg-primary/15'
              )}
            >
              <span className="text-base leading-none">{emojiFor(idx)}</span>
