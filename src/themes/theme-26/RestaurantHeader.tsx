...
export function RestaurantHeader({ orders = [], onViewOrder }: RestaurantHeaderProps) {
  ...
  return (
    <header className="relative">
      {/* Hero Banner — photo shows at full clarity; title gets a strong
          text-shadow so it stays readable on any background. */}
      <div className="relative h-56 sm:h-72 overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-10">
          <div className="container px-4 py-3 flex items-center justify-between">
            ...
            >
              <div className="w-12 h-12 rounded-full overflow-hidden shadow-lg border-2 border-white/60 ring-2 ring-[hsl(var(--brand-green))]/40">
                <img
            ...
            <button
              ...
              aria-label="Menu"
              className="relative w-11 h-11 rounded-full bg-white/95 hover:bg-white shadow-lg flex items-center justify-center text-[hsl(var(--brand-green))] transition-colors"
            >
              <MenuIcon className="w-5 h-5" />
              {orders.length > 0 && (
                <span
                  className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1.5 rounded-full bg-[hsl(var(--brand-green))] text-white text-[11px] font-bold flex items-center justify-center shadow-md ring-2 ring-white animate-pulse"
        ...
        >
          <h1 className="text-white text-4xl sm:text-5xl font-extrabold text-center px-6 tracking-tight leading-tight drop-shadow-[0_4px_18px_rgba(0,0,0,0.85)]">
            {restaurant.name}
          </h1>
        </motion.div>
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2">
          {!isRestaurantActive ? (
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-success text-black text-sm font-semibold rounded-full shadow-lg">
        ...
        <div className="overflow-hidden bg-card py-2 border-y border-border">
          ...
          >
            {[...Array(10)].map((_, i) => (
              <span
                key={i}
                className="text-base font-semibold text-[hsl(var(--brand-green))] mx-8 tracking-wide uppercase"
              >
                {restaurant.slogan1 || restaurant.slogan2}
              </span>
            ))}
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-border">
                <h3 className="text-xl font-bold text-[hsl(var(--brand-green))] tracking-tight">
                  {restaurant.name}
                </h3>
                <button
                  onClick={closeDrawer}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                  ...
                  <div className="space-y-2 mb-3 pb-3 border-b border-border">
                    ...
                    {orders.map((order) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-[hsl(var(--brand-green))] text-white shadow-md hover:opacity-90 transition-opacity text-left"
                      >
                ...
                {restaurant.reservationSettings?.isActive && (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-border hover:border-[hsl(var(--brand-green))] hover:bg-secondary transition-colors"
                  >
                    <CalendarDays className="w-5 h-5 text-[hsl(var(--brand-green))]" />
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-border hover:border-[hsl(var(--brand-green))] hover:bg-secondary transition-colors"
                >
                  <Star className="w-5 h-5 text-[hsl(var(--brand-green-bright))]" />
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isActive
                              ? "border-[hsl(var(--brand-green))] bg-[hsl(var(--brand-green))]/10 text-[hsl(var(--brand-green))]"
                              : "border-border hover:border-[hsl(var(--brand-green))]/40 text-muted-foreground hover:text-foreground",
                          )}
                        >
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isActive
                              ? "bg-[hsl(var(--brand-green))]/10 text-[hsl(var(--brand-green))] font-semibold"
                ...
                <div className="pt-3 border-t border-border space-y-2">
                  {restaurant.phoneNumber && (
                    ...
                    >
                      <Phone className="w-5 h-5 text-[hsl(var(--brand-green))]" />
                  ...
                  {(restaurant.latitude || restaurant.longitude) && (
                    <a
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-secondary transition-colors"
                    >
                      <MapPin className="w-5 h-5 text-[hsl(var(--brand-green))]" />
